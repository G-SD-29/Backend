...
// Die Factory baut den Server. serveStdio() ruft sie auf, sobald ein Client sich verbindet —
// so kann das SDK selbst entscheiden, welche Protokollversion (2025 oder 2026-07-28) gesprochen wird.
function createServer() {
  ...
  server.registerTool(
    'search_artworks',
    {
      ...
      // .describe() liefert dem LLM Hinweise zur Bedeutung der einzelnen Parameter,
      // was die Qualität der Tool-Aufrufe verbessert.
      // (In v1 ging hier auch ein Objekt ohne z.object() — in v2 ist das deprecated.)
      inputSchema: z.object({
    ...
    async ({ query, page }) => {
      try {
        ...
        // sondern als normales Ergebnis zurückgegeben — der Client (LLM)
        // bekommt die Fehlermeldung und kann darauf reagieren.
        // isError: true markiert das Ergebnis als Fehler, sonst hält der Client es für einen Erfolg.
        return {
          isError: true,
          content: [
            {
  ...
  server.registerResource(
    'documentation',
    // Für eine feste URI reicht ein String. Das Schema (hier "docs://") ist frei wählbar.
    // Ein ResourceTemplate braucht man nur für URIs mit Platzhaltern, z.B. "artworks://{id}".
    'docs://art-institute-of-chicago',
    {
...
// stdio nutzt stdin/stdout — der Client startet den Server als child process.

serveStdio(createServer);

// Testen mit dem MCP Inspector:
// npx @modelcontextprotocol/inspector node dist/app-current.js
//
// Testen in Postman:
// -> Neuer Request -> MCP statt HTTP
// -> STDIO /pfad/zu/node /pfad/zu/dist/app-current.js

// In VSCode oder anderem Client in der mcp.json (globale Konfig-Datei) hinzufügen - z.B.:
// "servers": {
// 	"mcp-aic": {
// 		"command": "node",
// 		"args": ["/absoluter/pfad/zu/05-mcp/dist/app-current.js"]
// 	}
// }
